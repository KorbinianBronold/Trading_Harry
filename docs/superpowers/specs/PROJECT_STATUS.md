...
| B-15 | `src/providers/capital_provider.py` | **Keine Session-Erneuerung nach 10 Minuten Inaktivität.** Capital.com verwirft die Session laut eigener API-Doku nach 10 Minuten ohne Aufruf; `_ensure_session()` prueft nur, ob ueberhaupt ein Token existiert, nie ob er noch gueltig ist — kein Retry-on-401 an den 9 Aufrufstellen, die `self._headers()` nutzen. Gefunden 2026-09-11 beim Live-Walkthrough (Phase 1d, `random/pipeline_walkthrough.ipynb`): der `price_provider` wird dort einmal instanziiert und ueber alle Phasen wiederverwendet, eine Diskussionspause > 10 min zwischen Zellen reicht fuer 401. In `pre_market`/`trade_proposals` gering riskant (Capital.com-Aufrufe clustern frueh im Lauf, danach nur noch Claude-Phasen); in `final_close` (sequenzielle Schleife ueber das volle Universum, eine Provider-Instanz) ungeprueft, aber real moeglich. **Bewusst nicht behoben (Entscheidung Korbinian) — der aktuelle Live-Einsatz ist Testen, kein Produktivlauf.** Workaround: Notebook-Kernel/Provider neu starten. | Niedrig (heute), zu beachten fuer `final_close` | nicht geplant |
